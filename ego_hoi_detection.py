...
# from tool_repos.ego_hand_detector.lib.model.utils.viz_hand_obj import *
# from tool_repos.ego_hand_detector.lib.model.rpn.bbox_transform import clip_boxes
# # from model.nms.nms_wrapper import nms
# from tool_repos.ego_hand_detector.lib.model.roi_layers import nms
# from tool_repos.ego_hand_detector.lib.model.rpn.bbox_transform import bbox_transform_inv
...
# from tool_repos.ego_hand_detector.lib.model.utils.blob import im_list_to_blob
# from tool_repos.ego_hand_detector.lib.model.faster_rcnn.resnet import resnet
...
def vis_detections_filtered_objects_PIL(im, obj_dets, hand_dets, thresh_hand=0.8, thresh_obj=0.01, 
                                        font_path='ego_hand_detector/lib/model/utils/times_b.ttf',
                                        repo_path='.'):
    ...
    image = Image.fromarray(im).convert("RGBA")
    draw = ImageDraw.Draw(image)
    # print(repo_path+'/'+font_path)
    font = ImageFont.truetype(repo_path+'/'+font_path, size=30)
    width, height = image.size 
    ...
    if (obj_dets is not None) and (hand_dets is not None):
        ...
        for hand_idx, i in enumerate(range(np.minimum(10, hand_dets.shape[0]))):
            ...
            if score > thresh_hand:
                ...
                bbox_obj = list(int(np.round(x)) for x in obj_dets[i, :4])
                score_obj = obj_dets[i, 4]
                image = draw_obj_mask(image, draw, hand_idx, bbox_obj, score_obj, width, height, font)
...
class EgoHOIDetection(object):
    ...
    def __init__(self, 
        ...
        if cfg_file is None:
            cfg_file = repo_path + '/ego_hand_detector/cfgs/res101.yml'
        else:
            cfg_file = repo_path + "/" + cfg_file
        self.repo_path = repo_path

        cfg_from_file(cfg_file)
        if set_cfgs is not None:
            cfg_from_list(repo_path + '/' + set_cfgs)
        ...
        if pretrained_path is None:
            pretrained_path = repo_path + '/ego_hand_detector/models/res101_handobj_100K/pascal_voc/faster_rcnn_1_8_132028.pth'
        else:
            pretrained_path = repo_path + "/" + pretrained_path
    ...
    @torch.no_grad()
    def detect(self, image_fp, vis=False):
        ...
        # )
        if vis:
            im2show = vis_detections_filtered_objects_PIL(im2show, obj_dets, hand_dets, 0.1, 0.2, repo_path=self.repo_path)
            # folder_name = save_dir
            # os.makedirs(folder_name, exist_ok=True)
            # result_path = os.path.join(folder_name, "vis_det.png")
            result_path = 'vis_det.png'
        ...
        if hand_dets[0, 9] == 1:
            hand_dets = np.flip(hand_dets, axis=0)
        if obj_dets[0, 9] == 1:
            obj_dets = np.flip(obj_dets, axis=0)
        result_dict = dict()
        for i, side_name in enumerate(['left', 'right']):
            hand_det = hand_dets[i]
            obj_det = obj_dets[i]
            side_dict = dict()
            side_dict['offset'] = hand_det[6:9]
            side_dict['bbox_obj'] = obj_det[:4]
            side_dict['bbox_hand'] = hand_det[:4]
            # side_dict['confidence_obj'] = obj_det[4]
            side_dict['confidence_hand'] = hand_det[4]
            side_dict['state'] = state_map2[int(hand_det[5])]
            side_dict['state_explaination'] = state_map[int(hand_det[5])]
            result_dict[side_name] = side_dict
            
        return result_dict, (h, w)

    def __call__(self, image_fp, vis=False):
        return self.detect(image_fp, vis)
    

if __name__ == "__main__":
    # base_fp = "/home/ycb/dflowmap_project/dflowmap/ps_datasets_epk/P02/rgb0.png"
    base_fp = "image.jpg"
    ego_hoi_detector = EgoHOIDetection(repo_path='.')
    result = ego_hoi_detector.detect(base_fp, vis=True)
    pdb.set_trace()